...
export default async function decorate(block) {
  ...
  try {
    ...
    resourceLibrarySearchEngine.subscribe(() => {
      ...
      const flattenedData = filteredData.flatMap((group) => (Array.isArray(group.pageData)
      ...
      renderCommonFacet(assetTypes, favoriteResultsList, (selected) => {
        ...
        const newFlattenedData = newFilteredData.flatMap((group) => (Array.isArray(group.pageData)
          ? group.pageData.map((item) => ({
            ...item,
            assetType: group.assetType,
          }))
